...
void Game::update(float dt)
{
    ...
    if (AI_MODE && agent) {
        ...
        float penalty = -0.1f;       // Phạt cực nhẹ để AI không bị trầm cảm
        float escape_reward = 0.2f;   // Thưởng vừa nếu lách khỏi viền đơn
        float super_escape = 0.4f;    // SIÊU THƯỞNG nếu biết đi chéo thoát góc chữ V!
    ...
    for (auto& e : enemies) {
        ...
        if (dist(pPos, e.getPosition()) < Player::RADIUS + Enemy::RADIUS) {
            if (player->getPower() >= e.getPower()) {
                ...
                player->gainPower(5.f);
                e.kill();
            }
            else {
                ev.took_damage = true;
                ev.damage_taken += (e.getPower() - player->getPower())*0.1f;
                ev.attacked_stronger = true;
                player->takeDamage((e.getPower() - player->getPower())*0.1f);
            }
        }
    }
}
...
void Game::spawnEnemies(int count) {
    if (player->getPower() >= (ENEMY_POWER_BUFF + 1) * 40) ENEMY_POWER_BUFF++;
    for (int i = 0; i < count; ++i) {
        float p = 5.f + float(std::rand() % 35) + ENEMY_POWER_BUFF*40;
